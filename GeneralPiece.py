...
class GeneralPiece(Piece):
    ...
    def is_legal(self, to_pos, board = None):
        ...
        if not super().is_legal(to_pos, board):
            return False

        # in the case that flying general isn't possible, check that the general remains in the castle
        if not flying_general_flag and to_pos not in (board.get_castle_spots(self._side)):
            return False
    ...
    def is_flying_general(self, to_pos, board = None):
        """ helper method to is_legal. Returns True if this general's move to to_pos is a flying general move"""
        if not board:
            board = self._board
        other_gen_pos = board.get_general_pos(self._opp)  # get the other general's position
        if other_gen_pos == to_pos and other_gen_pos[0] == self._pos[0]:  # if both generals on the same file
            path_to_gen = self.get_path(other_gen_pos, board)            # get ortho path to general
