class Piece:
    ...
    def get_path(self, to_pos, board = None):
        ...
        if not board:
            board = self._board
        try_ortho = board.get_ortho_path(self._pos, to_pos)
        try_diag = board.get_diagonal_path(self._pos, to_pos)
    ...
    def is_legal(self, to_pos, board = None):
        ...
        try_path = self.get_path(to_pos, board)
    ...
    def move(self, to_pos, board = None):
        ...
        if not board:
            board = self._board
        if not self.is_legal(to_pos, board):
            return False
        prev_pos = self._pos
